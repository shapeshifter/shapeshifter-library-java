...
import org.lfenergy.shapeshifter.connector.service.handler.annotation.UftpOutgoingHandler;
import org.springframework.stereotype.Service;

@Slf4j
@Service
public class UftpHandlerMapping {
  ...
  public UftpHandlerMapping(@UftpIncomingHandler List<Object> incomingHandlers,
  ...
  private void validateIncomingNotRegistered(Method receivingMethod, Class<? extends PayloadMessageType> type, Object uftpController) {
    if (incomingMethods.containsKey(type)) {
      ...
      throw new UftpConnectorException(
          getErrorMessage(existing, type, uftpController, receivingMethod, "incoming"));
    }
  }
  ...
  private void validateOutgoingNotRegistered(Method receivingMethod, Class<? extends PayloadMessageType> type, Object uftpController) {
    if (outgoingMethods.containsKey(type)) {
      ...
      throw new UftpConnectorException(
          getErrorMessage(existing, type, uftpController, receivingMethod, "outgoing"));
    }
  }
  ...
  private String getErrorMessage(UftpHandlerMethod existing, Class<? extends PayloadMessageType> type, Object uftpController, Method receivingMethod, String direction) {
    return "Method " + existing.bean().getClass().getName() + ":" + existing.method().getName() +
        " is already registered to handle " + direction + " type " + type.getName() +
