...
class XsdSchemaProviderTest {
  ...
  private void doTestXXE(String fileName, String errorMessage) {
    ...
    var xmlFile = this.getClass().getClassLoader().getResource(fileName);
    var xmlStreamSource = new StreamSource(new File(xmlFile.getFile()));
    assertThatThrownBy(() -> validator.validate(xmlStreamSource))
